...
const Starred = () => {
  ...
  useEffect(() => {
    if (vaultToken) fetchStarred();
  }, [vaultToken]);

  const fetchStarred = async () => {
    setLoading(true);
    try {
      const res = await axiosInstance.get('/vault/favorite-passwords', {
  ...
  return (
    <div>
      ...
      <div className={!vaultToken ? "blur-sm pointer-events-none select-none" : ""}>
        <div className="p-6">
          <h2 className="text-xl font-bold text-gray-800 mb-4">⭐ Starred Passwords</h2>

          {loading ? <p>Loading...</p> : Object.keys(grouped).length === 0 ? (
            <p className="text-gray-500 text-sm">No starred passwords found.</p>
          ) : (
