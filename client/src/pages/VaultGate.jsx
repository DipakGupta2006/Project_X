...
const VaultGate = ({ onVerified }) => {
    ...
    // Mount hone pe init karo
    useEffect(() => {
        initGate();
    }, []);

    // Verified hone pe parent ko token bhejo
    useEffect(() => {
        if (isVerified && vaultToken) {
            onVerified(vaultToken);
        }
    }, [isVerified, vaultToken]);

    const handleNext = () => setStep((prev) => prev + 1);

    const handleSubmit = () => verifyGate();
            ...
            <div className="bg-white rounded-2xl p-8 w-full max-w-md shadow-2xl">
                ...
                {error && (
                    <div className="bg-red-50 border border-red-200 text-red-600 text-sm rounded-lg px-4 py-2 mb-4">
                        {error}
                        {attemptsLeft < 3 && (
                            <span className="block font-semibold">{attemptsLeft} attempt(s) remaining</span>
                        )}
                    </div>
                )}
